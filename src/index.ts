...
const transformModFormat = (mod: IMod, activeProfile: Profile): Mod =>
    ({
        ...
        enabled: !!activeProfile.modState && activeProfile.modState[mod.id]?.enabled === true,
        vortexId: mod.id,
    });
...
const init = (context: IExtensionContext) => {
    ...
    const backupMods = (thisGameOnly: boolean = false, thisProfileOnly: boolean = false) => {
        return () => {
        ...
        api.selectFile({create: false, title: 'Select your backup file to import'})
            .then(fileName => {
                fs.readFile(path.resolve(fileName), async (error, jsonString) => {
                    ...
                    const modsToInstall = mods.filter(mod => !installedMods.some(
                    ...
                    const modsToChangeState = mods
                        ...
                        .map(mod => installedMods.filter(
                            // This will return true if the two mods (installedMod and mod) are the same
                            installedMod => installedMod.game === mod.game && installedMod.modId === mod.modId && installedMod.fileId === mod.fileId && installedMod.enabled !== undefined && installedMod.enabled !== mod.enabled
                        )[0]).filter(mod => !!mod);
                    ...
                    for (const mod of modsToInstall) {
                        if (shouldInstall && mod.enabled !== false) {
                            ...
                            const allowInstall = mod.enabled === false ? false : undefined;
                            api.emitAndAwait('nexus-download', mod.game, mod.modId, mod.fileId, undefined, allowInstall)
                        }
                    }

                    // Flip the enabled state for mods that aren't set correctly for this profile
                    modsToChangeState.forEach(mod => api.store.dispatch(actions.setModEnabled(selectors.activeProfile(api.getState()).id, mod.vortexId, !mod.enabled)));
    ...
    context.registerAction('mod-icons', 999, 'show', {}, 'Modlist Backup Only This Profile', backupMods(true, true));
};
